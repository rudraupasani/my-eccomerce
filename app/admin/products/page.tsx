...
import { getProducts, getCategories, createProduct, deleteProduct, updateProduct, uploadProductImage } from '@/lib/api'
...
export default function AdminProductsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // some Supabase errors nest message property
      console.error('Details:', error?.message || JSON.stringify(error))
      alert(`Failed to load product data. See console for details.`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmitProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Upload image if a new file has been chosen
      ...
      if (isEditingProduct && editingProductId) {
        ...
        const updates: any = {
          ...
          in_stock: inStock,
          category: categories.find(c => c.id === newCategoryId)?.name || 'Uncategorized'
      ...
      {isAddingProduct && (
        <ScrollAnimate type="scale-up">
          <div className="bg-card border border-border rounded-2xl p-8 shadow-sm">
            <h2 className="text-xl font-bold mb-6">{isEditingProduct ? 'Edit Product' : 'New Collection Item'}</h2>
            <form onSubmit={handleSubmitProduct}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-1 space-y-2">
                  ...
                  >
                    {imageFile ? (
                      ...
                      <img src={existingImageUrl} alt="Existing" className="w-full h-full object-cover rounded-lg absolute inset-0" />
                    ) : (
                      <>
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 content-start">
                  ...
                  {/* In Stock Checkbox */}
                  <div className="md:col-span-2 space-y-2">
                    <label className="flex items-center gap-3 cursor-pointer group p-3 bg-background border border-border rounded-lg hover:border-accent/50 transition">
                      <input
                        type="checkbox"
                        checked={inStock}
                        onChange={(e) => setInStock(e.target.checked)}
                        className="w-5 h-5 border border-border rounded accent-accent cursor-pointer"
                      />
                      <span className="text-sm font-semibold text-foreground group-hover:text-accent transition">In Stock</span>
                    </label>
                    <p className="text-[10px] text-muted-foreground ml-1">Check this box if the product is available for purchase</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 flex gap-3 pt-6 border-t border-border">
                <button type="submit" disabled={isSubmitting} className="btn-primary px-8 flex items-center justify-center min-w-[160px]">
                  {isSubmitting ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : (isEditingProduct ? 'Update Product' : 'Save to Catalog')}
                </button>
                <button
                  type="button"
              ...
              <tbody className="divide-y divide-border">
                {loading ? (
                  ...
                  <tr key={product.id} className="hover:bg-secondary/20 transition group">
                    ...
                    <td className="py-4 px-4">
                      <span className={`badge py-1 px-3 ${product.in_stock ? 'bg-emerald-500/10 text-emerald-500' : 'bg-destructive/10 text-destructive'
                        }`}>
                      ...
                      <div className="flex items-center justify-end gap-2 text-muted-foreground">
                        <button
                          onClick={() => {
                            ...
                            setImageFile(null) // keep existing until changed
